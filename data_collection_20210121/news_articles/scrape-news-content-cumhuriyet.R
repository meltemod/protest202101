...
get_content=function(link){
  ...
  #news_source
  news_source= page %>%
    html_nodes(".haberKaynagi > a")%>% 
    html_text()
  if(length(news_source)==0){news_source=''}
  
  #image-link
  image_link = page %>%
    html_nodes(".col-sm-8 > img")%>% 
    html_attr('src')
  if(length(image_link)==0){image_link=''}
  
  #image-text
  image_text = page %>%
    html_nodes(".col-sm-8 > alt")%>% 
    html_attr('src')
  if(length(image_text)==0){image_text=''}
  
  #text
  text= page %>%
    html_nodes(".haberMetni > p")%>% 
